    ...
    class KnowledgeTracer(nn.Module):
        def __init__(self, num_concepts: int, hidden_size: int = 128):
            ...
            self.knowledge_predictor = nn.Sequential(
                ...
                nn.Linear(64, num_concepts), nn.Sigmoid()
            )
        ...
        def forward(self, concept_seq, performance_seq):
            ...
            return self.knowledge_predictor(lstm_out[:, -1, :])
else:
    class KnowledgeTracer:
        """Lightweight placeholder used when PyTorch is not installed."""
        def __init__(self, num_concepts: int, hidden_size: int = 128):
            self.num_concepts = num_concepts
            self.hidden_size = hidden_size
...
class StudentModel:
    ...
    def initialize_knowledge_tracer(self, num_concepts: int):
        ...
        self.knowledge_tracer = KnowledgeTracer(num_concepts, hidden_size)
        if TORCH_AVAILABLE:
            self.knowledge_tracer = self.knowledge_tracer.to(self.device)
    ...
    def update_knowledge(self, current_knowledge: float, performance: float, time_spent: float) -> float:
        ...
        return float(np.clip(new_knowledge, 0, 1))
