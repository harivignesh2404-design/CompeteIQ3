...
class ProgressTracker:
    ...
    def record_learning_session(self, student_id: str, session_data: Dict[str, Any]):
        ...
        self.student_progress[student_id]['engagement_history'].append({
            'timestamp': session_record['timestamp'],
            'level': float(session_record['engagement_level'])
        })
    ...
    def get_student_insights(self, student_id: str) -> Dict[str, Any]:
        ...
        avg_performance = float(np.mean([session['performance'] for session in progress_data['learning_sessions']])) if progress_data['learning_sessions'] else 0.0
    ...
    def calculate_recent_engagement(self, student_id: str, days: int = 7) -> float:
        ...
        return float(np.mean([session['engagement_level'] for session in recent_sessions]))
